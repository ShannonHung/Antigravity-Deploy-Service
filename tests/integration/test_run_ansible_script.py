...
def test_debug_container_has_vault_env_preset(tmp_path):
    """Even a bare `-d` container (no playbook/inventory) gets the vault client
    mounted and ANSIBLE_VAULT_PASSWORD/_FILE preset, so manual runs decrypt."""
...
def test_vault_client_autogenerated_mounted_and_env_preset(tmp_path):
    """A sourced ANSIBLE_VAULT_PASSWORD triggers an auto-generated vault client
    (no secret inside) that is mounted into the container, with
    ANSIBLE_VAULT_PASSWORD_FILE preset and the password handed in by NAME only."""
    ...
    # The client was auto-generated, executable, and holds NO secret.
    assert vault_file.exists(), "vault client should be auto-generated"
    assert os.access(vault_file, os.X_OK), "vault client must be executable"
    body = vault_file.read_text()
    assert vault not in body, "generated client must not contain the password"
    assert "ANSIBLE_VAULT_PASSWORD" in body  # it reads the env var

    argv = (tmp_path / "docker_argv").read_text()
    # Mounted into the container and pointed at by ANSIBLE_VAULT_PASSWORD_FILE.
    assert f"-v {vault_file}:/ansible_vault:ro" in argv
    assert "-e ANSIBLE_VAULT_PASSWORD_FILE=/ansible_vault" in argv
    # Password handed in by NAME only — never as a value, never in argv.
    assert "-e ANSIBLE_VAULT_PASSWORD" in argv
    ...
    # No stdin wrapper anymore; ansible reads the mounted client automatically.
