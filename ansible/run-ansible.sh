...
# Where the auto-generated vault-password client is mounted INSIDE the container;
# ANSIBLE_VAULT_PASSWORD_FILE is pointed here so ansible finds it automatically.
VAULT_CLIENT_CONTAINER="/ansible_vault"
...
# Host path of the auto-generated vault-password client. Lives beside this script
...
Options:
  ...
  --secret-path <path>    KEY=VALUE env file sourced before the run. Recognised
                          ...
                          client (.ansible_vault, no secret inside) is generated
...
# Auto-generate the vault-password client next to this script (VAULT_PASS_FILE).
# It is created ONLY when a vault password is available. The file carries NO
# secret — it just echoes $ANSIBLE_VAULT_PASSWORD, which is handed to the
# container by value-less `-e`. It is (re)written every run so it can never
# drift, and mounted read-only into the container at VAULT_CLIENT_CONTAINER with
# ANSIBLE_VAULT_PASSWORD_FILE pointed at it — so both normal and debug containers
# have vault decryption wired up by default.
ensure_vault_client() {
  [[ -z "${ANSIBLE_VAULT_PASSWORD:-}" ]] && return 0
  cat > "$VAULT_PASS_FILE" <<'CLIENT'
#!/bin/sh
# Auto-generated by run-ansible.sh. Contains no secret: it prints the vault
# password supplied to the container via the ANSIBLE_VAULT_PASSWORD env var.
printf '%s' "$ANSIBLE_VAULT_PASSWORD"
CLIENT
  chmod 700 "$VAULT_PASS_FILE"
  echo ">> Vault client generated: $VAULT_PASS_FILE (mounted at $VAULT_CLIENT_CONTAINER)"
}
...
build_cmd_args() {
  ...
  # Vault: mount the auto-generated client and point ANSIBLE_VAULT_PASSWORD_FILE
  # at it, plus hand the password in by NAME only (value-less -e, so it stays out
  # of argv). ansible then decrypts automatically — no per-command flags, and it
  # works identically for a manual `ansible-playbook` inside a debug container.
  SECRET_ENV_ARGS=()
  VAULT_MOUNT_ARGS=()
  if [[ -n "${ANSIBLE_VAULT_PASSWORD:-}" ]]; then
    if [[ ! -f "$VAULT_PASS_FILE" ]]; then
      echo "Error: vault client not found at $VAULT_PASS_FILE (should have been auto-generated)." >&2
      exit 2
    fi
    SECRET_ENV_ARGS=(-e ANSIBLE_VAULT_PASSWORD -e "ANSIBLE_VAULT_PASSWORD_FILE=$VAULT_CLIENT_CONTAINER")
    VAULT_MOUNT_ARGS=(-v "$VAULT_PASS_FILE":"$VAULT_CLIENT_CONTAINER":ro)
  fi
...
══════════════════ RUN SUMMARY ══════════════════
  ...
  Vault          : $([[ -n "${ANSIBLE_VAULT_PASSWORD:-}" ]] && echo "ANSIBLE_VAULT_PASSWORD_FILE=$VAULT_CLIENT_CONTAINER (client auto-generated)" || echo "none")
  Log file       : $LOG_FILE
══════════════════════════════════════════════════
EOF
}
...
run_debug() {
  ...
  docker run -d --name "$DEBUG_CONTAINER" \
    ...
    ${inv_mount[@]+"${inv_mount[@]}"} \
    -v "$SSH_KEY":/root/.ssh/id_key:ro \
    ${VAULT_MOUNT_ARGS[@]+"${VAULT_MOUNT_ARGS[@]}"} \
    -e ANSIBLE_PRIVATE_KEY_FILE=/root/.ssh/id_key \
